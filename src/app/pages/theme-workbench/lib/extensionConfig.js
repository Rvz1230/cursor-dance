...
export async function readExtensionConfig() {
  ...
  if (!chromeApi?.storage?.local) {
    return normalizeStoredConfig(defaultConfig);
  }
...
export async function previewThemePack(themeId, themePack, actionId = "leftClick") {
  ...
  if (!chromeApi?.tabs?.sendMessage || site.tabId == null || !site.isSupportedPage) return false;
...
function buildDraftFromThemePack(themePack, siteMode) {
  ...
  const textContent = textEffect.content ?? baseDraft.actionConfigs.leftClick.textContent;
  ...
  return {
    ...
    actionConfigs: {
      ...
      leftClick: {
        ...
        textContent,
        textTags: Array.from(new Set([textContent, ...baseDraft.actionConfigs.leftClick.textTags].filter(Boolean))),
