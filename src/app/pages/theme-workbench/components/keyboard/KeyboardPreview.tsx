...
export function KeyboardPreview({ config, onUpdate, onCaptureChange }: {
  ...
  const stageHint = config.anchor === "caret"
    ? "插入点锚点下位置由插入点决定；距离可在右侧调整"
    : showHandle
      ? `点四周选择入场方向；拖动圆点调整${config.animationStyle === "raindrop" ? "起始" : "停留"}位置`
      : "这套组合下位置由键位与重力决定，可在右侧调整参数";

  return (
    <section className="keyboard-card keyboard-preview-card">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-4 py-3">
        <div className="flex items-center gap-2"><h2 className="text-sm font-medium text-slate-900">屏幕预览</h2>{focused ? <span className="rounded-full bg-sky-50 px-2 py-0.5 text-2xs font-semibold text-sky-700 ring-1 ring-sky-200">正在捕获按键 · Esc 退出</span> : null}</div>
        <div className="flex items-center gap-2">
          <div className="inline-flex rounded-xl border border-slate-200 bg-slate-50 p-1" role="radiogroup" aria-label="桌面底色">
            {(["light", "dark"] as const).map((value) => <button key={value} type="button" role="radio" aria-checked={wall === value} onClick={() => setWall(value)} className={cn("flex h-7 items-center gap-1 rounded-lg px-2 text-xs font-medium", wall === value ? "bg-slate-950 text-white" : "text-slate-500")}>{value === "light" ? <Sun className="size-3.5" /> : <Moon className="size-3.5" />}{value === "light" ? "浅色" : "深色"}</button>)}
          </div>
      ...
      {open ? <div className="px-4 py-3">
        <div className="keyboard-preview-stage relative mx-auto">
          <div ref={stageRef} data-keyboard-screen role="application" tabIndex={0} aria-label="屏幕预览：点一下再打字即可预览效果" onFocus={() => setCapture(true)} onBlur={() => setCapture(false)} onKeyDown={trigger} className={cn("relative aspect-[16/10] w-full overflow-hidden rounded-xl border outline-none transition-shadow", wall === "dark" ? "border-slate-700 bg-slate-900" : "border-slate-200 bg-slate-100", focused && "ring-2 ring-sky-400 ring-offset-2")}>
            <div className={cn("absolute inset-x-0 top-0 h-[6%]", wall === "dark" ? "bg-slate-950/80" : "bg-white/80")} />
            <div className={cn("absolute left-[6%] top-[14%] h-[48%] w-[36%] rounded-lg border", wall === "dark" ? "border-slate-700 bg-slate-800/80" : "border-slate-200 bg-white/80")}><div className={cn("h-[14%] border-b", wall === "dark" ? "border-slate-700" : "border-slate-200")} /></div>
            <div className={cn("absolute left-[52%] top-[26%] h-[56%] w-[42%] rounded-lg border", wall === "dark" ? "border-slate-600 bg-slate-800/70" : "border-slate-300 bg-white/70", config.anchor !== "screen" && "ring-2 ring-sky-400/60")}>
              <div className={cn("h-[13%] border-b", wall === "dark" ? "border-slate-600" : "border-slate-200")} /><div className="mx-[12%] mt-[14%] space-y-2"><div className={cn("h-1.5 w-4/5 rounded-full", wall === "dark" ? "bg-slate-600" : "bg-slate-300")} /><div className={cn("h-1.5 w-3/5 rounded-full", wall === "dark" ? "bg-slate-600" : "bg-slate-300")} />{config.anchor === "caret" ? <div className="ml-[55%] h-5 w-0.5 animate-pulse bg-sky-500" /> : null}</div>
            ...
            {config.originMapping === "keyboardLayout" && previewGeometry && (previewGeometry.edge === "top" || previewGeometry.edge === "bottom") && config.anchor !== "caret" ? (
              <div className="pointer-events-none absolute rounded-lg border border-dashed border-slate-400/60" style={{ left: previewGeometry.anchor.x, width: previewGeometry.anchor.width, height: Math.max(18, previewGeometry.fontSize * 1.6), top: Math.max(2, Math.min(stageSize.height - Math.max(20, previewGeometry.fontSize * 1.6), previewGeometry.point.y - previewGeometry.fontSize * 0.8)) }}>
                <span className="absolute left-1 top-1 text-2xs font-medium text-slate-500">按键位散开</span>
                {[[0.1, "q"], [0.28, "e"], [0.45, "t"], [0.62, "u"], [0.8, "o"], [0.92, "p"]].map(([x, label]) => <span key={String(label)} className="absolute top-1/2 -translate-x-1/2 -translate-y-1/2 font-bold text-slate-400/60" style={{ left: `${Number(x) * 100}%`, fontSize: previewGeometry.fontSize }}>{label}</span>)}
              </div>
            ) : null}
            {config.originMapping === "typewriter" && previewGeometry && (previewGeometry.edge === "top" || previewGeometry.edge === "bottom") && config.anchor !== "caret" ? (
              <div className="pointer-events-none absolute border-b border-dashed border-slate-400" style={{ left: previewGeometry.anchor.x + previewGeometry.anchor.width * 0.04, width: previewGeometry.anchor.width * 0.88, top: Math.max(8, Math.min(stageSize.height - 8, previewGeometry.point.y + previewGeometry.fontSize * 0.62)) }}><span className="absolute -top-5 left-0 text-2xs font-medium text-slate-500">打字机基线</span></div>
            ) : null}
          ...
          {(Object.keys(EDGE_LABELS) as Array<keyof typeof EDGE_LABELS>).map((edge) => <button key={edge} type="button" role="radio" aria-checked={(config.anchor === "caret" ? "bottom" : config.originEdge) === edge} disabled={config.anchor === "caret"} onClick={() => onUpdate({ originEdge: edge })} className={cn("keyboard-edge-button", `keyboard-edge-button--${edge}`, (config.anchor === "caret" ? "bottom" : config.originEdge) === edge && "keyboard-edge-button--active")}>{EDGE_LABELS[edge]}</button>)}
          {showHandle && previewGeometry ? <button type="button" aria-label={`拖动${config.animationStyle === "raindrop" ? "起始" : "停留"}位置`} onPointerDown={dragRestPoint} className="keyboard-rest-handle" style={{ left: previewGeometry.point.x, top: previewGeometry.point.y }}><span /></button> : null}
        </div>
        <div className="mt-2.5 flex min-h-6 items-center justify-between gap-3"><p className="min-w-0 text-2xs leading-relaxed text-slate-500">{stageHint}</p><div className="flex shrink-0 items-center gap-1.5"><span className="rounded-lg bg-slate-50 px-2 py-1 text-xs font-semibold tabular-nums text-slate-600 ring-1 ring-slate-200">同显 {effects.length}/{config.maxSimultaneous}</span>{config.typingCombo ? <span className="flex items-center gap-1 rounded-lg bg-slate-50 px-2 py-1 text-xs text-slate-500 ring-1 ring-slate-200">连击 {Array.from({ length: 5 }, (_, index) => <span key={index} className={cn("size-1.5 rounded-full", index < comboLevel ? "bg-slate-800" : "bg-slate-200")} />)}</span> : null}</div></div>
        {config.anchor === "caret" ? <p className="mt-2 rounded-lg bg-amber-50 px-2.5 py-2 text-2xs leading-relaxed text-amber-800 ring-1 ring-amber-200">中文输入法组字保护需要系统输入上下文能力；当前全局键盘钩子只能识别物理按键，桌面运行时会回落到可用锚点。</p> : null}
      </div> : null}
    </section>
  );
}
