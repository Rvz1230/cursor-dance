...
export function PreviewTimeline({ tracks, totalMs, disabled, canEditEmptyState, updateActionConfig, currentTimeMs, onSeek }: PreviewTimelineProps) {
  ...
  const [collapsed, setCollapsed] = useState(() => typeof window !== "undefined" && window.innerHeight < 560);
  ...
  return (
    <section className="shrink-0 border-t border-slate-200 bg-white shadow-[0_-2px_10px_rgba(15,23,42,0.04)] select-none">
      ...
      <div className="flex items-center justify-between gap-2 px-3 pb-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className="truncate text-xs text-slate-500">{tracks.map((track) => track.label).join(" · ") || "暂无轨道"}</span>
        </div>
