// 任务 3.0.5：把扩展端 extension/config.js 通过 IIFE 注入到 window 上的两个全局
// (CursorDanceDefaultConfig / CursorDanceConfigRuntime) 在桌面 renderer 里复刻一份。
//
// 背景：
//   - 扩展端 manifest 加载 extension/config.js → 在 window 上挂全局 → workbench 的
//     runtimeConfig.ts (getDefaultConfig / getRuntimeConfig) 直接读这俩全局。
//   - 桌面 workbench 渲染进程不会加载 extension/*，导致两个全局缺失，
