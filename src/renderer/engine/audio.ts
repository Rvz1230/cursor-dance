...
//     桌面端 overlay 没有页面 <audio>/<video>，无音量可压制；
//     soundBlendMode 在桌面端的语义留给上层后续讨论，引擎层不再做任何 duck。
//   - 5 个预设音效（woodfish-deep / tick-light / chime-bright / pop-soft /
//     swipe-whoosh）的波形 / 频率 / 持续时长 / 衰减时长**完全保留**，
//     与扩展版字节级一致。
//   - Web Audio API（OscillatorNode、GainNode、exponentialRampToValueAtTime…）
//     调用全部保留。
//
// 调用方在桌面端是 src/renderer/overlay；扩展端继续由 audio.js 注册。
...
export interface AudioRuntimeDeps {
  ...
  /** 可选 —— 扩展端把错误回写到 chrome.storage；桌面端可省略 */
...
export function createAudioRuntime(deps: AudioRuntimeDeps): AudioRuntimeModule {
  const { window, state, configStore, diagnostics, reportRuntimeError } = deps;
  ...
  function playSound(
    ...
    // 桌面版不再调 duckPageMedia —— overlay 没有页面媒体可压制。
