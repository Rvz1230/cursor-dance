...
- [ ] R4-2：建立 EffectRuntime adapters——桌面 InputSource、ContextResolver、EffectSurface 与 AudioOutput 均已接入生产链路；共享 action state machine 待继续迁移
- **R4-2 当前验证**：60 个根测试文件共 331 项通过；typecheck、lint（0 error，保留既有 24 warning）、Web/Electron build、Web smoke 5/5 和 desktop smoke 1/1 通过
- **下一步**：提取 timing、throttle、combo 与 output plan，让 trigger state machine 的决策层进入共享 core。Windows 同步执行 R1-4 真机验收。
