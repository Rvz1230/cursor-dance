...
- [x] 主题与效果工作台专项：动作页签展示真实启用效果数；八类效果卡统一为紧凑卡头、真实预设与渐进设置；实时预览接入背景、连打、轨迹、A/B、播放和自动重播；时间轴提升为跨列可折叠抽屉，支持吸附、缩放、拖动、逐帧与节奏复制。
