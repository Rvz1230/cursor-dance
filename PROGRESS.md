...
- **R4-4 当前进度**：effect lifecycle、DOM effect surface、cursor overlay、Web Audio 播放核心及扩展页面媒体 duck/profile 均已迁为 TypeScript；音频 IIFE 镜像已删除
- **下一步**：继续 R4-4 收敛 atmosphere 与剩余扩展装配入口；随后补真实 Chrome 扩展加载/CSP 验收。Windows 同步执行 R1-4 真机验收。
