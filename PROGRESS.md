...
- **R5-1 当前进度**：消息气泡、反馈交互与流式消息已提取为独立组件，并共用同一套 Markdown/桌面外链 renderer；`AiSchemePanel.tsx` 从 1,388 行降至 1,136 行，Workbench options bundle 同步减少约 0.6 kB。
...
- **下一步**：继续 R5-1，拆分提案展示和 Agent 时间线，再把请求生命周期收敛为独立 hook。Windows 同步执行 R1-4 真机验收。
