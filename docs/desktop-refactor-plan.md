...
| R5-1 拆分 `AiSchemePanel` | 进行中 | 消息、提案与 Agent 展示已拆分，重复 Markdown renderer 已合并；主文件由 1,388 行降至 735 行 |
...
- `AiSchemePanel.tsx` 从 1,388 行降至 735 行；新增 Web smoke 覆盖 AI 面板开关、初始消息、模式切换与输入面。
- 下一段收敛请求、取消、冷却与流式事件状态为独立 hook。
