...
`atmosphere` 保存不依赖单次点击动作的主题级效果。`atmosphere.trail` 是 Web 与桌面端共享的鼠标拖尾配置，包含启用状态、形态、采样长度、余辉时间、平滑度、柔光、速度响应、转向散射和手势爆发。`material` 支持 `neon / flame / ink / liquid / lightning / petal / note / code`；旧主题缺少该字段时使用 `neon`，并继续由 `shape` 决定光带、星尘、像素或残像几何。`blendMode` 支持 `normal / screen / soft-light / overlay`；`quality` 支持 `auto / eco / balanced / fine`，自动档从精细质量开始并在持续掉帧时逐级降档。`randomSeed` 固定粒子相位，使相同路径在预览、Web 和桌面端保持一致；`clickColor / clickDurationMs` 控制按下鼠标时整条可见轨迹的临时强调色与持续时间。`followCursorStateColor` 开启时复用当前光标状态绑定动作的 `cursorGlowColor`，没有状态色时回退到分段颜色，点击强调色优先级最高。`settleResponse / flickResponse / stopResponse / circleResponse` 分别控制停顿收束、快速甩动、急停涟漪和绕圈光环，设为 `0` 即关闭对应行为；旧配置只含 `gestureResponse` 时会把该值迁移到四个独立强度。`segments.tail / middle / head` 分别保存尾部、中段和光标附近的颜色、宽度与透明度；旧配置中的 `colors / width / opacity` 仍会被读取并自动推导三段样式，缺少新字段时使用默认值。网页端的磁场光晕继续使用同一对象中的 `mode`、`magnetRadius` 与 `magnetStrength`。

拖尾配方使用独立的 `cursordance-cursor-trail` 格式（当前 `version: 1`），只封装规范化后的 `trail` 对象。复制/粘贴可在主题之间复用，`.cursordance-trail.json` 导入导出不会覆盖主题中的其他效果。Web 与工作台预览通过 CSS 混合模式作用于页面背景；桌面透明覆盖窗无法跨应用读取背景像素，因此使用 Canvas 内部合成作为兼容回退。
